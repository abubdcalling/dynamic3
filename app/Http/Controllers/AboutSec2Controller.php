...
class AboutSec2Controller extends Controller
{
    ...
    public function show()
    {
        ...
        // Add full URL for img before sending response
        if ($aboutSec2 && $aboutSec2->img) {
            $aboutSec2->img = url('uploads/AboutSec2/' . $aboutSec2->img);
        }

        // Add full URL for video before sending response
        if ($aboutSec2 && $aboutSec2->video) {
            $aboutSec2->video = url('uploads/AboutSec2/' . $aboutSec2->video);
        }

        return response()->json($aboutSec2);
    }
    ...
    public function storeOrUpdate(Request $request)
    {
        ...
        // Attach full URLs for img and video before sending response
